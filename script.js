...
const ANSWER_LEVELS = [
  {
    ...
    badge: "초3",
    summary: "초등학교 3학년 수준",
    description: "동화책처럼 쉬운 말로 5줄 정도만 요약해요.",
    instruction: "답변 난이도는 쉬움입니다. 어린이가 동화책을 읽는다고 생각하고, 아주 쉽고 다정한 말로 설명하세요. 전체 답변은 5줄 정도의 완전 요약으로 쓰고, 긴 과학 설명은 넣지 마세요.",
    responseDetail: "관찰 근거는 눈에 보이는 단서 1~2가지만 고르고, 어려운 말은 어린이가 바로 알 수 있는 말로 바꿔 주세요.",
...
const EASY_FINAL_RESPONSE_RULES = [
  ...
  "전체 답변은 5개의 짧은 줄 정도로 끝내세요.",
  "각 줄은 한 문장으로 쓰고, 어린이에게 말하듯 아주 쉽게 쓰세요.",
  "맨 위 첫 줄은 반드시 '지형명: ○○ 크레이터'처럼 공식 지형명을 아주 짧게 쓰세요.",
  "공식 지형명이 확실하지 않으면 첫 줄에 '지형명: ○○ 부근으로 추정'처럼 추정이라고 분명히 쓰세요.",
  "크레이터 지형은 '크리에이터'가 아니라 '크레이터'라고 쓰세요.",
  "순서는 1) 정확한 지형명, 2) 어디인지, 3) 왜 만들어졌는지, 4) 왜 그렇게 보이는지, 5) 한 줄 기억하기입니다.",
  "관찰 근거는 눈에 보이는 단서 1~2가지만 쓰고, 과학적으로 중요한 점이나 긴 설명은 넣지 마세요.",
  "이름 유래는 꼭 필요할 때만 쉬운 말로 짧게 덧붙이세요.",
  "다음 관찰 질문은 만들지 마세요.",
].join("\n");
...
const EASY_FOLLOWUP_RESPONSE_RULES = [
  ...
  "답변에는 최종 설명만 한국어로 작성하세요.",
  "답변 앞뒤에 영어 계획표, 문단 설계, 자체 점검표, 내부 판단 메모를 절대 붙이지 마세요.",
  "전체 답변은 3~5개의 짧은 문장으로 끝내세요.",
  "공식 지형명이 질문과 관련 있으면 맨 위 첫 문장에 아주 짧게 먼저 쓰세요.",
...
function getAnalysisStructureInstruction(answerLevel, selectionType) {
  if (answerLevel.key === "easy") {
    return selectionType === "rover"
      ? "답변에는 1) 무엇처럼 보이는지, 2) 어디에 있는 사진인지, 3) 왜 생겼을 가능성이 있는지, 4) 왜 그렇게 보이는지, 5) 한 줄 기억하기를 동화책처럼 쉬운 5줄 요약으로 포함해 주세요."
      : "답변에는 1) 정확한 지형명, 2) 표시한 곳이 어디인지, 3) 왜 만들어졌는지, 4) 왜 그렇게 보이는지, 5) 한 줄 기억하기를 동화책처럼 쉬운 5줄 요약으로 포함해 주세요.";
  }
